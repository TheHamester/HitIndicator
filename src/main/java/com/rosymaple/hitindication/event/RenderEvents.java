...
public class RenderEvents {
    ...
    public static void onRender(RenderGuiOverlayEvent.Post event) {
        ...
        if(HitIndicatorClientConfigs.EnableProximityIndicators.get()) {
            ...
            for(Entity e : entities) {
                ...
                drawIndicator(event.getGuiGraphics(), indicator, screenMiddleX, screenMiddleY, playerPos, lookVec);
            }
        }
    ...
    private static float calculateDistanceFromPlayer(Vector3d location) {
        if(Minecraft.getInstance().player == null)
            return 0;

        Vec3 playerPos = Minecraft.getInstance().player.getPosition(0);
        double d0 = location.x - playerPos.x;
        double d1 = location.y - playerPos.y;
        double d2 = location.z - playerPos.z;
